const MenuCart = ({ item }) => {
    ...
    return (
        <div className="group relative flex flex-col lg:flex-row items-center gap-6 p-6 rounded-lg shadow-lg bg-white overflow-hidden transition-all hover:shadow-2xl duration-300">
            {/* Image Section */}
            <div
                className="w-full lg:w-40 lg:h-40 h-56 rounded-t-lg  overflow-hidden shadow-md group-hover:shadow-xl transition-transform duration-300 group-hover:scale-105"
            >
                <img
                    src={image}
                    alt={name}
                    className="w-full h-full object-cover lg:object-center rounded-t-lg lg:rounded-full group-hover:brightness-110"
                />
            </div>
            {/* Content Section */}
